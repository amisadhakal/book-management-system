...
namespace BookManagement.Application.DTOs
{
    ...
    // Bulk / cart checkout — same customer details but a list of book IDs.
    public class CartCheckoutDto
    {
        [Required(ErrorMessage = "At least one book must be selected.")]
        [MinLength(1)]
    ...
    // Result for a bulk cart purchase.
    public class CartOrderResultDto
    {
        public List<OrderDto> Succeeded { get; set; } = new();
        public List<FailedBookDto> Failed { get; set; } = new();
        public decimal TotalPaid { get; set; }
    }

    public class FailedBookDto
    {
        public int BookId { get; set; }
        public string BookTitle { get; set; } = string.Empty;
